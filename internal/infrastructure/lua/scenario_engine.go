...
func (e *ScenarioEngine) ExecuteFunction(functionName string) error {
	logger.Debug("[DEBUG] ExecuteFunction called: %s\n", functionName)
	...
	if fn == lua.LNil {
		logger.Debug("[DEBUG] Function %s not found in Lua state\n", functionName)
		return errors.WrapLuaExecutionFailed(functionName, fmt.Errorf("function %s not found", functionName))
	}

	logger.Debug("[DEBUG] Executing Lua function: %s\n", functionName)
	e.state.Push(fn)
	if err := e.state.PCall(0, 0, nil); err != nil {
		logger.Debug("[DEBUG] Lua function execution failed: %v\n", err)
		return errors.WrapLuaExecutionFailed(functionName, err)
	}

	logger.Debug("[DEBUG] Lua function executed successfully\n")
	return nil
}
